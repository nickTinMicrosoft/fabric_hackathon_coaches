...
Part of the **MS Azure Days — Fabric Hackathon coaches toolkit**, this simulator streams synthetic water quality sensor readings to Azure Event Hub for Microsoft Fabric Real-Time Intelligence (RTI) demos.
...
Add the following to the project root `.env` file (`C:\repos\fabric_hackathon_coaches\.env`):
...
The script follows the same root `.env` loading pattern used by the Hospital and Metra simulators in this repository.
...
```bash
cd C:\repos\fabric_hackathon_coaches\Scripts\Python\WaterQuality
...
See the `docs\` and `kql\` folders included here for the full hot path, warm path, Power BI, and KQL setup guidance used in the coaches repo.
